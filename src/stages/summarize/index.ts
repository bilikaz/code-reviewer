...
interface VerdictDecision {
  ...
  unaddressed: ThreadDecision[];
  infoFindings: ReviewFinding[];
  noReviewableSource: boolean;
}

function deriveVerdict(ctx: Ctx, state: StageState): VerdictDecision {
  ...
  let unaddressed: ThreadDecision[];
  ...
  const verdict: Verdict =
    blocking.length || unaddressed.length || noReviewableSource ? "reject" : "approve";
...
function totals(state: StageState): { tokens: number; healAttempts: number } {
  let tokens = 0, healAttempts = 0;
  for (const m of state.metrics) {
    if (m.tokens) tokens += m.tokens.total;
    healAttempts += m.healAttempts ?? 0;
  }
  return { tokens, healAttempts };
}

export async function runSummarize(ctx: Ctx, state: StageState): Promise<StageState> {
  const t0 = Date.now();
