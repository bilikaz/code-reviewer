// The LLM client — everything consumers call. Two entry points:
//
//   callLLM()      — one "call" per stage. Internally a loop: stream a chat
//                    completion → if it returned tool_calls, execute them
//                    locally and feed the results back, then re-stream. When
//                    the LLM returns final content (no tool_calls), parse the
//                    JSON, validate against the schema, and either return or
//                    heal.
//   resolveModel() — model auto-detection via GET /models (used by buildCtx
//                    when no model is configured).
//
// Wire-level plumbing lives below this file in transport.ts (message types,
// HTTP POST, SSE parsing).

import { Ajv } from "ajv";

import type { Ctx } from "../ctx.ts";
import { errorMessage } from "../lib/errors.ts";
import { receive, send } from "./transport.ts";
import { execTool, TOOL_SCHEMAS } from "./tools/index.ts";
import type { ChatMessage, LLMResult, Section } from "./types.ts";
...
function extractJson(text: string): Extracted {
  ...
  for (const c of candidates) {
    ...
    catch (e) { if (!parseError) parseError = errorMessage(e); }
  }
...
export async function callLLM<T = unknown>(args: CallLLMArgs): Promise<LLMResult<T>> {
  ...
  for (let attempt = 0; attempt <= healRetries; attempt++) {
    ...
    while (true) {
      ...
      const resp = await send({ baseUrl, apiKey, req });
      const msg = await receive(resp, { onText: (s) => ctx.logger.stream(s) });
      if (msg.usage) {
    ...
    try {
      ...
      lastError = `JSON parse: ${errorMessage(e)}`;
    }
    if (extracted) {
...
// Ask the endpoint via `GET /models` and pick the first id. Only called when
// no explicit model is configured (buildCtx branches before calling); no
// hardcoded provider heuristics — if the endpoint yields nothing, error out
// with a pointer to --llm-model.
