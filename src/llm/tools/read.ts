...
export const readTool: Tool = {
  schema: {
    ...
    function: {
      name: "Read",
      description:
        "Read lines from a file in the working directory. By default returns the " +
  ...
  async execute(args) {
    ...
    if (count <= 0) count = DEFAULT_LINES;
...
function formatRead(path: string, content: string, start: number, count: number): string {
