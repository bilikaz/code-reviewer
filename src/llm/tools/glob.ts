...
export const globTool: Tool = {
  schema: {
    ...
    function: {
      name: "Glob",
      description:
        "Find files whose path matches a glob pattern, e.g. `src/**/*.ts` or " +
  ...
  async execute(args) {
    ...
    if (!pattern) {
      return {
        ok: false,
        output:
          `Glob call rejected: missing required \`pattern\` argument. ` +
    ...
    try {
      ...
      const isExcluded = (p: string) => p.split(/[\\/]/).some((seg) => EXCLUDE.includes(seg));
      for await (const m of glob(pattern, { exclude: isExcluded })) {
        matches.push(m);
        if (matches.length >= MAX_RESULTS) break;
      }
      if (matches.length === 0) return { ok: true, output: `No files match ${JSON.stringify(pattern)}.` };
