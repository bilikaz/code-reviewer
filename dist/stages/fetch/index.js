...
import { firstHunkNewLine, headLinesCovered, runGit } from "../../lib/git.js";
import { loadStandards, renderStandards } from "./standards.js";
import { emptyRenderingContext, } from "../types.js";
const BINARY_SIZE_SIMILARITY_THRESHOLD = 0.8;
export async function runFetch(ctx) {
    const t0 = Date.now();
    ...
    if (pc.source && existsSync(pc.source)) {
        ...
        if (pc.sections.length > 0)
            inlineProjectContext = sliceSections(inlineProjectContext, pc.sections);
    }
    ...
    if (ctx.config.review.reviewChecklistPath) {
        ...
        if (existsSync(p))
            reviewChecklist = readFileSync(p, "utf-8");
        else
            log.warn("checklist.missing", { path: p });
    }
...
async function assembleRendering(ctx, meta, changed, log) {
...
function pushDiff(bucket, path, type, content) {
    bucket.push({ path, type, content, lineCount: content.split("\n").length, size: content.length });
}
function tryFileSize(path) {
    try {
        if (!existsSync(path))
            return 0;
...
async function tryOldSize(baseSha, path) {
    try {
...
function countLinesOnDisk(path) {
    try {
        if (!existsSync(path))
            return null;
...
function readFirstNLines(path, n) {
    try {
