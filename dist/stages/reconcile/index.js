...
function splitThreads(comments) {
    const auto = [];
    const rest = [];
    for (const c of comments) {
        if (c.by !== "bot" || c.resolved || c.parentId)
            continue;
        if (!c.inline)
            continue;
        if (c.inline.path && !existsSync(c.inline.path))
            auto.push(c);
        else
            rest.push(c);
    }
...
function gatherReplies(comments, threadIds) {
    return comments.filter((c) => c.parentId && threadIds.has(c.parentId));
}
function hasRenameNotice(comments, newPath, oldPath) {
    const expectedPrefix = `${RENAME_BANNER_PREFIX}\`${oldPath}\` → \`${newPath}\``;
    for (const c of comments) {
        if (c.by !== "bot")
            continue;
        if (!c.inline || c.inline.path !== newPath || c.inline.line !== 1)
            continue;
        if ((c.body || "").startsWith(expectedPrefix))
            return true;
    }
    return false;
}
async function handleRenames(ctx, comments, renames) {
    if (!Object.keys(renames).length)
        return 0;
    const log = ctx.logger.child("reconcile.rename");
    let count = 0;
    for (const c of comments) {
        if (c.by !== "bot" || c.resolved)
            continue;
        if (!c.inline)
            continue;
        const oldPath = c.inline.path;
        if (!oldPath || !(oldPath in renames))
            continue;
        ...
        try {
            if (!alreadyPosted) {
                const notice = `${RENAME_BANNER_PREFIX}\`${oldPath}\` → \`${newPath}\`. ` +
            ...
            log.error("relocate.failed", { thread: c.id, oldPath, newPath, error: e.message });
        }
    }
    return count;
}
async function applyResolutions(ctx, byCommentId, decisions) {
    ...
    for (const d of decisions) {
        ...
        try {
            ...
            log.error("resolve.failed", { thread: comment.threadId, error: e.message });
            failed++;
        }
    }
...
export async function runReconcile(ctx, state) {
    const t0 = Date.now();
    ...
    if (Object.keys(renames).length > 0) {
        ...
        if (relocated > 0)
            comments = await ctx.vcs.getPRComments();
    }
    const { auto, rest } = splitThreads(comments);
    const byCommentId = new Map(comments.map((c) => [c.id, c]));
    ...
    const autoDecisions = auto.map((c) => ({
    ...
    if (auto.length > 0)
        log.info("auto_addressed", { count: auto.length });
