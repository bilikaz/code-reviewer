...
import { receive, send } from "./transport.js";
...
export async function callLLM(args) {
    ...
    throw new LLMValidationError(`stage=${args.stage} exhausted ${healRetries} heal attempts; last: ${lastError}`);
}
export async function resolveModel(args) {
    ...
    if (!resp.ok) {
        ...
        throw new Error(`model auto-detection failed: GET ${url} → HTTP ${resp.status}. ` +
            `Pass --llm-model explicitly or set LLM_MODEL. ${text.slice(0, 200)}`);
    }
    const data = (await resp.json());
    const first = data.data?.[0]?.id;
    if (!first) {
        throw new Error(`model auto-detection failed: ${url} returned no models. ` +
