...
export const bashTool = {
    schema: {
        ...
        function: {
            name: "Bash",
            description: "Run a shell command in the working directory. Returns combined stdout+stderr plus the exit code. Use for `git log`, `git blame`, `git show`, `grep -rn`, `find`, listing dirs, etc. The container is throwaway — no path restrictions, but state does not persist between calls.",
            parameters: {
                type: "object",
    ...
    async execute(args) {
        ...
        if (!command) {
            return {
                ok: false,
                output: `Bash call rejected: missing required \`command\` argument. ` +
