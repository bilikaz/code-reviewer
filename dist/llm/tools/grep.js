...
export const grepTool = {
    schema: {
        ...
        function: {
            name: "Grep",
            description: "Search file contents for a pattern (regular expression), like `grep -rn`. " +
    ...
    async execute(args) {
        ...
        if (!pattern || !path) {
            return {
                ok: false,
                output: `Grep call rejected: both \`pattern\` and \`path\` are required. ` +
        ...
        if (!existsSync(path)) {
            return {
                ok: false,
                output: `Grep call rejected: path ${JSON.stringify(path)} does not exist. ` +
                    `Pass a file or directory that exists (use Ls or Glob to locate it).`,
            };
        }
        const r = await runGrep(pattern, path);
        if (r.timedOut)
            return { ok: false, output: `Grep timed out searching ${path}.` };
        if (r.code === 0)
            return { ok: true, output: cap(r.output) };
        if (r.code === 1)
            return { ok: true, output: `No matches for ${JSON.stringify(pattern)} in ${path}.` };
...
function runGrep(pattern, path) {
    ...
    return new Promise((resolve) => {
        ...
        proc.stderr.on("data", (b) => { out += b.toString("utf-8"); });
        proc.on("close", (code) => {
            clearTimeout(timer);
