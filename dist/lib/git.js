...
export async function gitFileDiff(opts) {
    // Three-dot syntax: diff between merge-base(base,head) and head — matches
    // what GitHub/GitLab show on the PR.
    return runGit(["diff", `-U${opts.context}`, `${opts.baseSha}...${opts.headSha}`, "--", opts.path], opts.cwd);
}
export async function gitChangedFiles(args) {
    ...
    const raw = await runGit(["diff", "-M", "--name-status", `${args.baseSha}...${args.headSha}`], args.cwd);
    const out = [];
    for (const line of raw.split("\n")) {
        ...
        const code = parts[0];
        if (code === "A")
            out.push({ path: parts[1], status: "added" });
        else if (code === "M")
            out.push({ path: parts[1], status: "modified" });
        else if (code === "D")
            out.push({ path: parts[1], status: "deleted" });
        else if (code.startsWith("R"))
            out.push({ path: parts[2], status: "renamed", oldPath: parts[1] });
        // T (type change), C (copy), U (unmerged) — rare on PR diffs, skip.
    }
    return out;
}
export async function gitPotentialRenames(args) {
    const raw = await runGit(["diff", "-M30", "--name-status", `${args.baseSha}...${args.headSha}`], args.cwd);
    const out = [];
    for (const line of raw.split("\n")) {
        ...
        if (!Number.isFinite(score) || score >= 50)
            continue; // ≥50 = confident, already handled
...
export function headLinesCovered(diff) {
    let inHunk = false;
    let count = 0;
...
export function firstHunkNewLine(diff) {
